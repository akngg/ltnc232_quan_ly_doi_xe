...
const Path = () =>{
    ...
    const getStationArr=async ()=>{
        try{
            const data = await getDocs(collection(database,"station"));
    ...
    const deleteStationArr=async(id)=>{
        try{
            const carDoc=doc(database, "station", id);
    ...
    const deleteGoods=async(id,station)=>{
        try{
            const carDoc=doc(database, "goods", id);
            await deleteDoc(carDoc);
    ...
    const deletePassenger=async(id,station)=>{
        try{
            const carDoc=doc(database, "passenger", id);
            await deleteDoc(carDoc);
            displayPopupBox(station);
        }catch(error){
            const errorCode = error.code;
    ...
    const displayPopupBox= async(station)=>{
        ...
        const queryDriver=query(collection(database, "drivers"),where("position","==",station.name),where("status","==","Active"));
        ...
        const queryIncomingDriver=query(collection(database, "drivers"),where("dest","==",station.name),where("status","==","Running"));
        ...
        const queryTruck=query(collection(database, "trucks"),where("position","==",station.name),where("status","==","Active"));
        ...
        const queryIncomingTruck=query(collection(database, "trucks"),where("dest","==",station.name),where("status","==","Running"));
        ...
        const queryBus=query(collection(database, "buses"), where("position","==",station.name),where("status","==","Active"));
        ...
        const queryIncomingBus=query(collection(database, "buses"), where("dest","==",station.name),where("status","==","Running"));
        ...
        const queryGoods=query(collection(database, "goods"),where("position","==",station.name), where("isMoving","==",false));
        ...
        const queryPassenger=query(collection(database, "passenger"),where("position","==",station.name), where("isMoving","==",false));
        // ,where("isMoving","==",false)
    ...
    const updatePerMinute=async()=>{
        ...
        const queryTruck=query(collection(database, "trucks"),where('arriveTime',">", 0), where('arriveTime',"<=", time));
                ...
                while(true){
                    ...
                    truck.arrayOfGoods.forEach(async(goods)=>{
                        if(goods.dest==truck.position){
                            ...
                            const goodsDoc=doc(database,"goods", goods.id);
                    ...
                    if(truck.arrayOfDests.length!=0){
                        ...
                        const driverDoc=doc(database, "drivers", truck.driver.id);
                        updateDoc(driverDoc,{
                    ...
                    const truckDoc=doc(database, "trucks", truck.id);
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        const queryBus=query(collection(database, "buses"),where('arriveTime',">", 0), where('arriveTime',"<=", time));
        const bus=await getDocs(queryBus);
        const filteredBus = bus.docs.map((doc)=>({
            ...doc.data(),
            id:doc.id,
                ...
                while(true){
                    ...
                    bus.arrayOfPassenger.forEach(async(passenger)=>{
                        if(passenger.dest==bus.position){
                            bus.passengers-=1;
                            const passengerDoc=doc(database,"passenger", passenger.id);
                    ...
                    if(bus.arrayOfDests.length!=0){
                        ...
                        const driverDoc=doc(database, "drivers", bus.driver.id);
                        updateDoc(driverDoc,{
                    ...
                    const busDoc=doc(database, "buses", bus.id);
                    ...
                    <button onClick={()=>{
                        ...
                        pathCalcTruckUsed.forEach(async (truck)=>{
                            ...
                            truck.arrayOfGoods.forEach(async(good)=>{
                                const goodsDoc=doc(database,"goods", good.id);
                    ...
                    <button onClick={()=>{
                        ...
                        pathCalcBusUsed.forEach(async (bus)=>{
                            ...
                            bus.arrayOfPassenger.forEach(async(pass)=>{
                                const passDoc=doc(database,"passenger", pass.id);
